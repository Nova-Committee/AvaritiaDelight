...
public class ADEmiPlugin implements EmiPlugin {
    ...
    public void register(EmiRegistry registry) {
        ...
        for (ExtremeCookingPotShapedRecipe recipe : registry.getRecipeManager().getAllRecipesFor(ExtremeCookingPotShapedRecipe.Type.INSTANCE))
            registry.addRecipe(new ExtremeCookingPotCategory(recipe));
        for (ExtremeCookingPotShapelessRecipe recipe : registry.getRecipeManager().getAllRecipesFor(ExtremeCookingPotShapelessRecipe.Type.INSTANCE))
            registry.addRecipe(new ExtremeCookingPotCategory(recipe));
        ...
        for (CropExtractorRecipe recipe : registry.getRecipeManager().getAllRecipesFor(CropExtractorRecipe.Type.INSTANCE))
